...
Private Sub pw_OLEDragDrop(Data As DataObject, Effect As Long, Button As Integer, Shift As Integer, x As Single, y As Single)
'OLE拖动
 Dim TF As Variant
...
If pW = "" Then
  pW = InputBox("请输入目标文件夹名:", "暂时解除伪装，使能正常存取文件", fuck)
  If pW = "" Then Exit Sub
End If
SetAttr pW, 0
End Sub
...
If pW = "" Then
  pW = InputBox("请输入目标文件夹名:", "恢复伪装状态，保护文件夹不被非法预览", fuck)
  If pW = "" Then Exit Sub
End If
SetAttr pW, vbSystem
End Sub
...
Select Case iNdex
 Case 1
  MsgBox "本程序是 璐绥居士 编写的，仅限私人使用，擅自传播者拖出去打！", vbExclamation + vbSystemModal, "关于"
 Case 2
  End
End Select
End Sub
